...
// Reducer
function cartReducer(state, action) {
  const { items, discounts, subtotal, total } = calculateDiscounts(state);

  function pushHistory(newState) {
    return {
      ...newState,
      history: [...state.history, { ...state, history: [], future: [] }],
      future: [],
    };
  }
  ...
  switch (action.type) {
    case "ADD_ITEM": {
      ...
      const { discounts, subtotal, total } = calculateDiscounts({ ...state, items });
      return pushHistory({ ...state, items, discounts, subtotal, total });
    }
    ...
    case "RESET_CART": {
      return pushHistory({ ...initialState });
    }
    ...
    case "UNDO": {
      ...
      return {
        ...
        future: [{ ...state, history: [], future: [] }, ...state.future],
      };
    }
    ...
    <div>
      ...
      {state.items.length > 0 && (
        <>
          ...
          <button disabled={state.history.length === 0} onClick={() => dispatch({ type: "UNDO" })}>
            Undo
          </button>
          <button disabled={state.future.length === 0} onClick={() => dispatch({ type: "REDO" })}>
            Redo
          </button>
        </>
      )}
...
export default function App() {
  return (
    <CartProvider>
      <h1>Shopping Cart with Discounts (Undo/Redo)</h1>
      <div style={{ display: "flex", gap: "40px" }}>
        <ProductList />
        <Cart />
        <CartSummary />
